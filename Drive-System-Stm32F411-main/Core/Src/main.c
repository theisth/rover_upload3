  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
      if (rxfull == 1){
		...
		speed_left = (linear_pwm_value - angular_pwm_value) + 1525;
		speed_right = (linear_pwm_value + angular_pwm_value) + 1525;



        DC_Control(linear_pwm_value, angular_pwm_value,&htim1);
      }
  }
  /* USER CODE END 3 */
}
